...
public class ZhihuBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        if (dy >=10&&!isAnimate&&child.getVisibility()==View.VISIBLE) {
            hide(child);
        } else if (dy <10&&!isAnimate&&child.getVisibility()==View.GONE) {
            show(child);
        }
    }
    ...
    private void hide(final View view) {
        ViewPropertyAnimator animator = view.animate().translationY(viewY).setInterpolator(INTERPOLATOR).setDuration(1000);
